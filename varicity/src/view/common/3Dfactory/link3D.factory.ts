...
export class Link3DFactory {
    public static createLink(src: Building3D, dest: Building3D, type: string, scene: Scene, config: Config): Link3D {
        ...
        if(config.link.display.air_traffic.includes(type)) return new Link3DImplem(src, dest, type, scene);
        if(config.link.display.underground_road.includes(type)) return new UndergroundRoad3DImplem(src, dest, type, scene);
        // switch(type) {
        //     // case 'DUPLICATES':
